...
const UserModal = ({
  ...
  const handleConfirm = async () => {
    if (onConfirm) {
      if (!isDeleteMode && (!editedUser.email || !editedUser.full_name || !editedUser.type)) {
        return toast.error("Email,password and name is required")
      }
      if (isInviteMode) {
  ...
  return (
    <div>
      {button ? (
        ...
        <Button onPress={onOpen} className="flex bg-emerald-500 text-white items-center gap-x-1">
          {
